...
The dashboard token is entered by the operator. The EdgeProxy token is used only by the SecurityEdge backend and is not exposed to browser JavaScript. Both credentials must be valid UTF-8 values used as single `Authorization: Bearer <token>` fields and no larger than 8192 UTF-8 bytes after normalization: surrounding whitespace is normalized, but oversized credentials, embedded whitespace, or control characters are rejected during validation so a configuration cannot pass startup checks with a credential outside the supported HTTP authentication contract. The literal `[REDACTED]` value is reserved for secret-preservation responses and cannot be configured as a real token. Structured Dashboard secret fields enforce the same contract before a mutation is sent; blank or whitespace-only inputs preserve the current redacted secret instead of accidentally clearing it.
