...
Control Plane writes require valid UTF-8 JSON, use strict decoding, reject unknown fields and bodies larger than 4 MiB, validate the complete candidate, preserve a supplied `[REDACTED]` Admin token, create timestamped backups, and atomically replace the configuration file. Invalid UTF-8 is rejected before JSON decoding so malformed request bytes cannot be silently converted to replacement characters and persisted. Route and Origin lookup is case-insensitive; route names are immutable after creation so SecurityEdge policy identities cannot silently drift. Route and Origin names cannot be the exact dot-segments `.` or `..` because those identifiers occupy Admin API URL path segments and would otherwise be canonicalized by HTTP routing. A route must retain at least one Origin.
...
`EDGEPROXY_ADMIN_TOKEN` always takes precedence over the JSON credential. Use the same strong value in `apps/edgeproxy/.env` and `apps/securityedge/.env`; SecurityEdge uses it only for authenticated backend calls to the EdgeProxy Admin API. Admin credentials are single `Authorization: Bearer <token>` values that must be valid UTF-8 input but normalize to printable ASCII credentials with a maximum normalized size of 8192 bytes: surrounding Go-style whitespace is normalized, while non-ASCII values, oversized credentials, embedded whitespace, or control characters are rejected during configuration validation because browsers and HTTP clients cannot reliably represent arbitrary UTF-8 bytes in the supported `Authorization: Bearer <token>` contract. The literal value `[REDACTED]` is reserved for Control Plane secret-preservation responses and is rejected as a real credential so file/environment configuration cannot collide with that API sentinel.
