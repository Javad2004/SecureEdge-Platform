...
try {
  ...
  let telemetryTrendManyGapContract = null;
  ...
  if (fixtureRoot) {
    telemetryTrendGapContract = await cdp.evaluate(`(() => {
      ...
      const requestLineOps = operations.filter(operation => operation.color === requestColor && operation.dash === '' && (operation.type === 'move' || operation.type === 'line'));
      const blockedLineOps = operations.filter(operation => operation.color === blockedColor && operation.dash === '' && (operation.type === 'move' || operation.type === 'line'));
      ...
      const result = {
        ...
        noteLabels:Array.from(document.querySelectorAll('#trend-chart-summary .trend-status-item strong')).map(node => node.textContent),
    ...
    if (telemetryTrendGapContract.requestLatest !== 'Unavailable' ||
        ...
        telemetryTrendGapContract.windowDetail !== '7 samples' ||
        telemetryTrendGapContract.requestCoverage !== '3 rate intervals available from 7 samples' ||
        ...
        telemetryTrendGapContract.noteCount !== 2 ||
        !telemetryTrendGapContract.noteLabels.includes('Gaps') ||
        !telemetryTrendGapContract.noteLabels.includes('Availability') ||
        !telemetryTrendGapContract.summary.includes('1 telemetry gap') ||
        !telemetryTrendGapContract.summary.includes('EdgeProxy request rate unavailable for 3 observed intervals') ||
        !telemetryTrendGapContract.summary.includes('SecurityEdge rejection rate unavailable for 2 observed intervals') ||
    ...
    telemetryTrendManyGapContract = await cdp.evaluate(`(() => {
      const previous = state.overview;
      const baseTime = Date.now() - 10 * 60 * 1000;
      // Fifteen regular 10-second intervals keep the inferred cadence stable;
      // four deliberately longer breaks exercise the dense-gap rendering path.
      const offsets = [
        0,10000,20000,30000,
      ...
      const samples = offsets.map((offset, index) => ({
        generated_at:new Date(baseTime + offset).toISOString(),
        security:{rejected_rate_available:true,rejected_per_second:index % 5 === 0 ? 0.1 : 0},
        edgeproxy:{available:true,request_rate_available:true,requests_per_second:0.25 + (index % 4) * 0.05}
      }));
      const candidate = structuredClone(previous);
      candidate.telemetry_history = {samples};
      state.overview = candidate;
      ...
      const requestColor = cssColor('--chart-requests', '#67a6ff');
      const blockedColor = cssColor('--chart-blocked', '#ff6b84');
      const requestLineOps = operations.filter(operation => operation.color === requestColor && operation.dash === '' && ['move','line'].includes(operation.type));
      const blockedLineOps = operations.filter(operation => operation.color === blockedColor && operation.dash === '' && ['move','line'].includes(operation.type));
      const gaps = trendTemporalGaps(state.trend);
      const result = {
        gapCount:gaps.length,
        gapDurations:gaps.map(gap => gap.duration),
        gapMarkers:operations.filter(operation => operation.type === 'text' && operation.text === '//').length,
        gapLabels:operations.filter(operation => operation.type === 'text' && operation.text.startsWith('Telemetry gap ·')).map(operation => operation.text),
        requestMoves:requestLineOps.filter(operation => operation.type === 'move').length,
        blockedMoves:blockedLineOps.filter(operation => operation.type === 'move').length,
        summary:document.getElementById('trend-chart-summary').textContent
      };
      state.overview = previous;
      renderAll();
    ...
    if (telemetryTrendManyGapContract.gapCount !== 4 ||
        telemetryTrendManyGapContract.gapMarkers !== 4 ||
        telemetryTrendManyGapContract.gapLabels.length !== 1 ||
        telemetryTrendManyGapContract.gapLabels[0] !== 'Telemetry gap · 2m' ||
        telemetryTrendManyGapContract.requestMoves !== 5 ||
        telemetryTrendManyGapContract.blockedMoves !== 5 ||
        !telemetryTrendManyGapContract.summary.includes('4 telemetry gaps') ||
        !telemetryTrendManyGapContract.summary.includes('longest 2m') ||
        !telemetryTrendManyGapContract.summary.includes('never zero-filled')) {
    ...
    if (telemetryTrendOutlierContract.model.clipped !== true || telemetryTrendOutlierContract.model.outlierCount !== 1 ||
        ...
        !telemetryTrendOutlierContract.scaleDetail.includes('1 peak') ||
        ...
        telemetryTrendOutlierContract.blockedCoverage !== 'No rejections · 20 rate intervals available from 20 samples' ||
    ...
    if (telemetryTrendZeroBaselineContract.requestLineOps !== 5 || telemetryTrendZeroBaselineContract.blockedLineOps !== 5 ||
        ...
        telemetryTrendZeroBaselineContract.requestCoverage !== 'No requests · 5 rate intervals available from 5 samples' ||
        telemetryTrendZeroBaselineContract.blockedCoverage !== 'No rejections · 5 rate intervals available from 5 samples' ||
    ...
    telemetryTrendLegendSemanticsContract = await cdp.evaluate(`(() => {
      const previous = state.overview;
      const baseTime = Date.now() - 28 * 5000;
      ...
      const requestCoverageNode = document.getElementById('trend-requests-coverage');
      const blockedCoverageNode = document.getElementById('trend-blocked-coverage');
      const style = getComputedStyle(requestCoverageNode);
      const requestRect = requestCoverageNode.getBoundingClientRect();
      const result = {
        requestCoverage:requestCoverageNode.textContent,
        blockedCoverage:blockedCoverageNode.textContent,
        whiteSpace:style.whiteSpace,
        textOverflow:style.textOverflow,
        overflowX:style.overflowX,
        overflowWrap:style.overflowWrap,
        requestHeight:requestRect.height,
        requestScrollWidth:requestCoverageNode.scrollWidth,
        requestClientWidth:requestCoverageNode.clientWidth,
    ...
    await cdp.send('Emulation.setDeviceMetricsOverride', {width:1365,height:900,deviceScaleFactor:1,mobile:false});
    if (telemetryTrendLegendSemanticsContract.requestCoverage !== 'No requests · 27 rate intervals available from 28 samples' ||
        telemetryTrendLegendSemanticsContract.blockedCoverage !== 'No rejections · 27 rate intervals available from 28 samples' ||
        telemetryTrendLegendSemanticsContract.whiteSpace !== 'normal' ||
        telemetryTrendLegendSemanticsContract.textOverflow !== 'clip' ||
        telemetryTrendLegendSemanticsContract.overflowX === 'hidden' ||
        !['anywhere','break-word'].includes(telemetryTrendLegendSemanticsContract.overflowWrap) ||
        telemetryTrendLegendSemanticsContract.requestHeight <= 20 ||
        telemetryTrendLegendSemanticsContract.requestScrollWidth > telemetryTrendLegendSemanticsContract.requestClientWidth + 1 ||
    ...
    if (telemetryTrendPartialAvailabilityLegendContract.requestCoverage !== 'No requests in observed intervals · 3 rate intervals available from 7 samples' ||
        ...
        !telemetryTrendPartialAvailabilityLegendContract.summary.includes('Color-coded dashed markers') ||
        ...
        !telemetryTrendPartialAvailabilityLegendContract.temporalGapSummary.includes('missing time spans remain blank') ||
    ...
    telemetryTrendSeriesAvailabilityMarkerContract = await cdp.evaluate(`(() => {
      const previous = state.overview;
      ...
      const samples = Array.from({length:120}, (_, index) => ({
        ...
        edgeproxy:{available:true,request_rate_available:true,requests_per_second:index >= 80 && index <= 92 ? 2.2 + (index % 2) * 0.2 : 0}
      }));
      const candidate = structuredClone(previous);
      candidate.telemetry_history = {samples};
      state.overview = candidate;
      ...
      let wideOperations = [];
      let compactOperations = [];
      try {
        wideOperations = drawAtWidth(640);
        compactOperations = drawAtWidth(390);
      } finally {
        canvas.getContext = originalGetContext;
        canvas.style.width = originalCanvasWidth;
        canvas.style.maxWidth = originalCanvasMaxWidth;
      ...
      const displayTrend = trendDisplayPoints(state.trend);
      const gapIndexes = new Set(trendTemporalGaps(displayTrend).map(gap => gap.beforeIndex));
      const requestRuns = trendSeriesUnavailableRuns(displayTrend, 'requests', gapIndexes);
      const blockedRuns = trendSeriesUnavailableRuns(displayTrend, 'blocked', gapIndexes);
      const markerLines = (operations, color) => operations.filter(operation =>
        operation.color === color && operation.dash === '3,3' && (operation.type === 'move' || operation.type === 'line'));
      const markerLabels = (operations, prefix) => operations.filter(operation =>
        operation.type === 'text' && operation.text.startsWith(prefix)).map(operation => operation.text);
      const markerSymbols = (operations, color) => operations.filter(operation =>
        operation.type === 'text' && operation.color === color && operation.text === '×').length;
      const result = {
        temporalGapCount:trendTemporalGaps(displayTrend).length,
        requestRunCount:requestRuns.length,
        blockedRunCount:blockedRuns.length,
        blockedRunIntervals:blockedRuns.reduce((total, run) => total + run.count, 0),
        wideRequestMarkerLines:markerLines(wideOperations, requestColor).length,
        wideBlockedMarkerLines:markerLines(wideOperations, blockedColor).length,
        wideRequestMarkerSymbols:markerSymbols(wideOperations, requestColor),
        wideBlockedMarkerSymbols:markerSymbols(wideOperations, blockedColor),
        ...
        compactBlockedLabels:markerLabels(compactOperations, 'SecurityEdge rejection rate unavailable'),
        requestCoverage:document.getElementById('trend-requests-coverage').textContent,
        blockedCoverage:document.getElementById('trend-blocked-coverage').textContent,
        summary:document.getElementById('trend-chart-summary').textContent,
        noteLabels:Array.from(document.querySelectorAll('#trend-chart-summary .trend-status-item strong')).map(node => node.textContent)
    ...
    if (telemetryTrendSeriesAvailabilityMarkerContract.temporalGapCount !== 0 ||
        telemetryTrendSeriesAvailabilityMarkerContract.requestRunCount !== 0 ||
        telemetryTrendSeriesAvailabilityMarkerContract.blockedRunCount !== 1 ||
        telemetryTrendSeriesAvailabilityMarkerContract.blockedRunIntervals !== 1 ||
        telemetryTrendSeriesAvailabilityMarkerContract.wideRequestMarkerLines !== 0 ||
        telemetryTrendSeriesAvailabilityMarkerContract.wideBlockedMarkerLines !== 2 ||
        telemetryTrendSeriesAvailabilityMarkerContract.wideRequestMarkerSymbols !== 0 ||
        ...
        telemetryTrendSeriesAvailabilityMarkerContract.noteLabels.includes('Gaps') ||
        !telemetryTrendSeriesAvailabilityMarkerContract.summary.includes('SecurityEdge rejection rate unavailable for 1 observed interval') ||
        !telemetryTrendSeriesAvailabilityMarkerContract.summary.includes('Color-coded dashed markers') ||
    ...
    if (telemetryTrendSeriesIsolationContract.sparseModel.clipped !== false ||
        ...
        !telemetryTrendSeriesIsolationContract.summary.includes('exact values remain in the scale summary')) {
    ...
    if (telemetryTrendLatestAvailabilityContract.points !== 4 ||
        ...
        telemetryTrendLatestAvailabilityContract.windowDetail !== '4 samples' ||
        ...
        telemetryTrendLatestAvailabilityContract.requestCoverage !== '1 rate interval available from 4 samples' ||
        telemetryTrendLatestAvailabilityContract.blockedCoverage !== '3 rate intervals available from 4 samples' ||
        !telemetryTrendLatestAvailabilityContract.summary.includes('never zero-filled')) {
    ...
    if (telemetryTrendEmptyContract.points !== 1 || telemetryTrendEmptyContract.emptyHidden !== false ||
        ...
        telemetryTrendEmptyContract.windowDetail !== '1 sample' ||
        !telemetryTrendEmptyContract.summary.includes('No interval rates are available yet across 1 sample')) {
