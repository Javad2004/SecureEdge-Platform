...
func TestDashboardPreservesAuthAndTelemetryTruthfulness(t *testing.T) {
	...
	for _, required := range []string{
		...
		"values remain blank and are never zero-filled.",
		"missing time spans remain blank and are never zero-filled.",
...
func TestDashboardPolicyEditorMatchesBackendContract(t *testing.T) {
	...
	for _, required := range []string{
		...
		`Policy validated and saved. A graceful SecurityEdge restart is scheduled; live values stay pinned until the new generation is active.`,
		`Policy accepted; restart scheduled`,
		...
		`reserved [REDACTED] secret marker`,
		`cannot exceed 8192 UTF-8 bytes`,
		...
		`const utf8Bytes = value => new TextEncoder()`,
		`function validateRouteNameCandidate(value, originalName = '')`,
		`Route name cannot be . or .. because it is used as an Admin API path segment.`,
		`Route name ${name} already exists.`,
		`function validateOriginCandidate(route, candidate, originalName = '')`,
		`Origin name cannot be . or .. because it is used as an Admin API path segment.`,
		`Origin name cannot exceed 256 UTF-8 bytes.`,
		`Route name cannot exceed 256 UTF-8 bytes.`,
		`id="route-max-response-header" type="number" min="1" max="16777216"`,
		`$('cache-config-form').reportValidity()`,
...
func TestDashboardPreservesControlPlaneEditorState(t *testing.T) {
	...
	for _, required := range []string{
		...
		"Policy applied after the graceful SecurityEdge restart.",
		"reconcilePendingRestartEditors();",
		"submitted values stay pinned until the new generation is active.",
		"submitted JSON stays pinned until the new generation is active.",
