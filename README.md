...
- The Dashboard includes a filterable Security Event Explorer and an EdgeProxy completed-request explorer with trusted Client IP, Request ID, Route/method/status/cache filters, configurable page size, and cursor-based older/newer navigation through the SecurityEdge backend-for-frontend; EdgeProxy Admin credentials remain server-side.
- HTTP/1.1 protocol upgrades such as WebSocket are preserved end-to-end through SecurityEdge and EdgeProxy. Upgrade requests bypass the shared HTTP cache, validate the selected protocol on both hops, preserve any client protocol bytes already read ahead during HTTP parsing, and switch to a long-lived bidirectional tunnel without inheriting normal HTTP request or server read/write deadlines after the `101` handshake. Active upgraded tunnels are explicitly tracked and closed when either managed service generation is retired, so clients should reconnect after an automatic configuration restart instead of remaining attached to obsolete runtime state.
...
Production containerization is intentionally separate from the local/demo Compose files. The repository provides three **standalone** Linux production manifests under [`deployments/docker-production/`](deployments/docker-production/README.md): EdgeProxy-only, SecurityEdge-only, and the complete EdgeProxy + SecurityEdge platform. All three use isolated bridge networking and an independent `/srv/secureedge` data root by default; they do not require systemd units, host service accounts, or an earlier SecureEdge installation. The combined mode publishes only SecurityEdge publicly, keeps the EdgeProxy data plane private, and routes EdgeProxy to a real Origin such as a Tailscale/VPN endpoint.

The production definitions use dedicated multi-stage Dockerfiles, non-root numeric identities, Docker secrets, read-only TLS mounts and root filesystems, dropped capabilities, configurable CPU/memory/PID limits, rotated container logs, health checks, and bootstrap/TLS/route/network/config validation helpers. Fresh hosts are first-class; an existing systemd installation can be imported only through an explicit optional migration helper. The combined profile defaults to HTTPS on both public ingress and the SecurityEdge → EdgeProxy data hop. The local/demo Dockerfiles and Compose manifests remain separately supported for development and self-contained demonstrations.
