...
The dashboard token is entered by the operator. The EdgeProxy token is used only by the SecurityEdge backend and is not exposed to browser JavaScript. Both credentials begin as valid UTF-8 input but normalize to printable ASCII values used as single `Authorization: Bearer <token>` fields and no larger than 8192 bytes after normalization: surrounding Go-style whitespace is normalized, while non-ASCII credentials, oversized values, embedded whitespace, or control characters are rejected so a configuration cannot pass startup checks with a credential that browsers or HTTP clients cannot represent byte-for-byte. The literal `[REDACTED]` value is reserved for secret-preservation responses and cannot be configured as a real token. Structured Dashboard secret fields enforce the same contract before a mutation is sent; blank or whitespace-only inputs preserve the current redacted secret instead of accidentally clearing it.
...
SecurityEdge treats the referenced EdgeProxy configuration as the authoritative shared Route table and applies the same file-level encoding and Route-selector boundaries as EdgeProxy itself: the complete JSON document must be valid UTF-8 before decoding, Route names are limited to 256 UTF-8 bytes, the shared table is limited to 2,048 Routes and 256 host patterns per Route, host patterns use the same canonical normalization, and `path_prefix` values must be canonical paths. A single trailing slash such as `/api/` is normalized to `/api`, while repeated trailing slashes such as `/api//`, dot-segments, repeated internal slashes, query/fragment markers, and percent-encoded path bytes are rejected instead of being silently collapsed. These mirrored limits prevent malformed or oversized selector data from creating a split-brain state where SecurityEdge accepts a shared Route table that EdgeProxy rejects or performs materially more validation work than the authoritative proxy.
...
When NDJSON persistence is enabled, SecurityEdge restores the newest retained events from the active file and configured rotated backups during startup. Event sequence numbers continue monotonically across restarts; malformed, invalid-UTF-8, crash-truncated, or oversized lines are skipped and counted in the log-store `file_errors` metric instead of preventing startup. Invalid UTF-8 is rejected before JSON decoding so damaged audit fields cannot be silently replacement-normalized into different Route, request, or event identifiers. Oversized records are discarded only through their terminating newline, so valid events later in the same active or rotated file are still recovered. Restored timestamps, rule IDs, tags, and WAF match details are normalized to the same bounded representation used by live events, so a damaged or manually altered log cannot inflate the in-memory ring through oversized nested fields.

The process-wide security-event ring accepts a configured `capacity` from `1` through `100000` entries and remains active even when the Admin listener is disabled, so its allocation and persistence limits are always validated. The ring remains fully available when `admin.log_store.file_path` is empty; in that memory-only mode file-size and backup-rotation settings are intentionally inactive. Configuring an NDJSON path enables persistence, at which point the Dashboard enables the file-size/backup controls and restores safe defaults if they were unset. Persistent security logs are limited to 1 GiB per file and at most 100 rotated backups. Persistent event-log and telemetry-history files, including log rotations and telemetry recovery staging, are required to remain isolated from one another and from SecurityEdge configuration, the referenced EdgeProxy Route configuration, their retained/staging backups, the loaded SecurityEdge dotenv file, and any configured SecurityEdge TLS certificate/key paths. These TLS paths remain reserved even while TLS is temporarily disabled, and a configured telemetry-history destination plus its recovery path remain reserved while history collection is paused, so an active event log cannot consume dormant managed data before a later re-enable. The loaded dotenv path is also rejected if it occupies a configuration staging or retained-backup namespace, so Control Plane backup rotation cannot remove or replace the environment source. Startup, validation, and restart preflight fail closed on aliases or path collisions so persistence and configuration maintenance cannot overwrite Control Plane, rollback, environment, TLS, or paused-history material. Gateway concurrency, rate-limit buckets, automatic-ban tracking, rates, and bursts are capped at 1,000,000; automatic-ban thresholds are capped at 10,000. Security policy collections are bounded as well: at most 2,048 Route policies, 256 custom WAF rules, 4,096 entries in each IP allow/deny list, 4,096 trusted-proxy CIDRs, and a 4 KiB regular-expression pattern per custom rule. Expired bans and stale partial-violation records are removed opportunistically, so bounded tracking capacity is reclaimed even without capacity pressure. SecurityEdge upstream connection pools are also capped at 1,000,000 configured slots per transport setting. Validation stops processing each oversized collection at its published ceiling, so invalid profiles cannot amplify CPU or diagnostic-memory use while they are being rejected. These boundaries prevent structurally valid configuration from requesting unbounded CPU, memory, disk, or rotation work.
...
Rate-limit buckets and automatic-ban tracking are also process-wide stores. Route policies may define different request rates, bursts, violation thresholds, windows, and ban durations, but they must use the same `cleanup_interval`, `idle_ttl`, `max_buckets`, and `max_tracked_clients` capacity settings as `default_policy`. This prevents one route from applying a smaller shared-store capacity to buckets or client records created by another route. When these process-wide values are edited through the structured Default Policy endpoint or Dashboard form, SecurityEdge propagates the inherited copies into every stored Route override, validates and atomically persists the complete candidate, returns `202 Accepted`, and schedules a graceful generation restart; the active generation keeps its previous limiter/ban stores until the replacement generation starts. Route-policy editors keep these values read-only and inherited. The case-insensitive identifier `__unmatched__` is reserved for SecurityEdge's internal unmatched-request telemetry and cannot be used as an EdgeProxy Route name or a SecurityEdge Route-policy key, preventing unmatched traffic from inheriting or merging with a user-defined Route identity.

Dashboard editors follow the same feature-dependent validation rules as the backend, including the Bearer-token 8192-byte, whitespace/control-character, and reserved-redaction-marker contract. Cache, active-health, request-body-inspection, rate-limit, and temporary-ban dependent settings are disabled when their feature is off and restore safe defaults when re-enabled. Structured System forms also disable inactive TLS, Admin, connectivity, DNS, telemetry-history, and EdgeProxy log-store children, restore safe numeric/duration defaults when an operator re-enables a bounded subsystem, and require certificate/DNS inputs only when the corresponding feature actually needs them. The browser also enforces backend cross-field relationships before a mutation is sent: per-client concurrency cannot exceed the global concurrency limit, log-store page sizes must fit within one another and the retained capacity, cache objects cannot exceed the total cache budget, active-health timeout cannot exceed its probe interval, and connectivity timeout/staleness plus telemetry-history cadence must remain within their configured timing boundaries. Go-style duration fields used by those relationships accept compound values such as `1h30m` and are rejected client-side when malformed. Route/Origin names are checked against the backend's 256-byte UTF-8 boundary and cannot be the exact dot-segments `.` or `..`, because those identifiers are used as Admin API URL path segments. Cache/health HTTP status-code lists are validated as integer `100..599` values before a mutation is sent. Origin edits are rejected client-side before mutation when the URL is not absolute HTTP/HTTPS, contains credentials or a fragment, uses insecure TLS skipping on plain HTTP, duplicates another Origin name, or duplicates another Origin URL on the same Route. The backend independently enforces the same safety boundary. If a full EdgeProxy configuration update persists Route metadata together with a restart-required listener/TLS change, SecurityEdge defers activation of the changed shared Route table until EdgeProxy reports that the replacement generation has applied the new revision; hot Route/Origin edits continue to synchronize immediately.
...
They produce structured JSON, load the proper Admin token from the process environment or application `.env`, reject invalid request JSON locally, fail fast on HTTP/API errors, and disable ambient proxy use for local or LAN management traffic. SecurityEdge's Admin API and EdgeProxy-forwarding BFF require request bodies to be valid UTF-8 before JSON decoding, matching the EdgeProxy Control Plane and preventing malformed bytes from being silently replaced before a mutation is applied.
