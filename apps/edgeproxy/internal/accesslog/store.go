...
func normalize(entry Entry) Entry {
	...
	entry.Method = truncate(strings.ToUpper(strings.TrimSpace(entry.Method)), 32)
	...
	entry.CacheStatus = truncate(strings.ToUpper(strings.TrimSpace(entry.CacheStatus)), 32)
