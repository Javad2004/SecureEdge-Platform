...
For CI, restricted sandboxes, or workstations whose browser policy blocks local HTTP navigation, fixture mode loads the real embedded Dashboard HTML and JavaScript into a real browser, supplies deterministic API responses from the checked-in local profiles, and verifies login, initial rendering, complete Route forms, per-route cache management, navigation, responsive layouts, Light/Dark contrast, system-default theme selection, persisted theme bootstrap, cross-tab storage synchronization, storage clearing fallback, and browser runtime errors:
